...
def partitioning(df_last):
    ...
    if number_of_rows_left <= 80:
        ...
        # Filter DataFrame based on 'Priority' column
        port_df = unfilled_df[unfilled_df['Priority'].str.startswith('P_Port')]
        #port_df = df[unfilled_df['Priority'].str.startswith('P_Port')]
        ...
        #other_unnamed_df = df[~df['Priority'].str.startswith('P_Port')]
        ...
        if len(port_df) < 80 :
            ...
            # Update 'Side' column in df only for rows that match port_df
...
def assigning_side_for_less_than_80_pin_count(df):
    ...
    df_Part['Side'] = df_Part.apply(lambda row: side_allocation(row, df_Part), axis=1)
